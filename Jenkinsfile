pipeline {
  ...
  post {
    ...
    success {
      echo """
      🎉 Java 17 Docker 빌드 성공!
      
      ✅ 빌드 번호: ${BUILD_NUMBER}
      ✅ Docker 이미지: ${DOCKER_IMAGE}:${BUILD_NUMBER}
      ✅ Java 버전: OpenJDK 17 (Docker Container)
      ✅ 배포 네임스페이스: ${NAMESPACE}
      ✅ 배포 시간: ${new Date()}
    ...
    failure {
      ...
      💡 문제 해결 가이드:
      1. Docker-in-Docker 권한 확인
      2. Docker Hub 자격증명 확인  
      3. kubeconfig 설정 확인
      4. Spring Boot 3.x와 Java 17 호환성 확인
      """
    }
